...
char names[M];
int n, k;
int recognizeNo(int current) {
    if(current == 0) return 0;
    int sum = 0;
    if(current - k < 0) {
        rep(i, current) {
            if(names[i] == names[current]) sum++;
        }
    } else {
        FOR(i, current - k, current - 1)
        if(names[i] == names[current]) sum++;
    }
    return sum;
}
...
int main() {
    ...
    while(tcase--) {
        ...
        mem(visited, 0);
        rep(i, n) {
            if(visited[names[i] - 'A'])
                result++;
            visited[names[i] - 'A']++;

            if ( i - k >= 0 ) visited[names[i - k] - 'A']--;
        }
        pf("Case %d: %d\n", ++caseNo, result);
    }
    return 0;
